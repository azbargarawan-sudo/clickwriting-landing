...
const AR_FONT = process.env.AR_FONT || 'Traditional Arabic';
const EN_FONT = process.env.EN_FONT || 'Times New Roman';
const FONT = { ascii: EN_FONT, hAnsi: EN_FONT, cs: AR_FONT, eastAsia: EN_FONT };
const BODY = 28;   // 14pt
const BODY_CS = 32; // 16pt for Arabic script (Traditional Arabic renders small)
...
const TITLE = 'دور الألعاب الحركية في تعليم الرياضيات للمرحلة الابتدائية وتحسين الفهم والتحصيل';
...
function cell(text, { header = false, width }) {
  return new TableCell({
    ...
    children: [new Paragraph({
      ...
      children: [new TextRun({ text, font: FONT, size: 24, sizeComplexScript: 28, bold: header, boldComplexScript: header, rightToLeft: true })],
    })],
  });
}
...
const cover = [
  coverLine('[اسم الجامعة / الكلية]', { bold: true, size: 30 }),
  coverLine('[اسم الكلية أو القسم]', { size: 28 }),
  coverLine('[اسم المساق]', { size: 26, after: 1400 }),
  coverLine(TITLE, { bold: true, size: 40, after: 400 }),
  coverLine('The Role of Kinesthetic Games in Teaching Mathematics at the Primary Stage and Improving Understanding and Achievement', { size: 26, after: 1400 }),
  coverLine('بحث مقدَّم استكمالاً لمتطلبات المساق', { size: 26, after: 600 }),
  coverLine('إعداد الطالب/ة: [الاسم الكامل]', { size: 28 }),
  coverLine('الرقم الجامعي: [الرقم]', { size: 28 }),
  coverLine('إشراف: [اسم المشرف/ة]', { size: 28, after: 800 }),
  coverLine('العام الدراسي 2025 / 2026', { size: 28 }),
];
