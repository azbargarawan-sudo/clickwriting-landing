...
const LANG = { value: process.env.DOC_LATIN_LANG || 'en-US', bidirectional: process.env.DOC_BIDI_LANG || 'ar-SA' };
...
const coverLines = process.env.DOC_COVER_FILE ? fs.readFileSync(process.env.DOC_COVER_FILE, 'utf8').split('\n').filter(l => l.trim() !== '') : [TITLE];
const cover = coverLines.map(l => {
  if (l.startsWith('!')) return coverLine(l.slice(1), { bold: true, size: 36, before: 400, after: 400 });
  if (l.startsWith('#')) return coverLine(l.slice(1), { bold: true, size: 30, after: 200 });
  if (l === '~') return coverLine(' ', { size: 24, after: 600 });
  return coverLine(l, { size: 26, after: 200 });
});
