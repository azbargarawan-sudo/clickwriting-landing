...
S[11] = [prose([
    ...
    'היא נועדה. ופאא, בת 54, ניסחה זאת במפורש: ',
    ('"אני יודעת בדיוק מה צריך לעשות. ובכל זאת עברו שנתיים"', 'i'),
    '. הגורם המעכב שזוהה הוא החשש מפני האבחנה ולא מפני הבדיקה עצמה, כפי שהבחינה איבתיסאם, בת 52: ',
    ...
    'החברתית בדרך אל הבדיקה, כפי שתיארה חנאן, בת 47: ',
...
S[12] = [prose([
    ...
    'של הסעה ושל נטל טיפולי בבית. ברכנש, בת 58, העמידה את שני הצדדים זה מול זה: ',
...
S[15] = [
    prose(['ההמלצות נגזרות מן הממצאים, ואינן מכוונות לשינוי אמונותיהן של הנשים אלא ',
...
S[16] = [prose([
    ...
    'אובדן מסוים של ניואנס. רביעית, זהותן של ארבע המראיינות כסטודנטיות למנהל מערכות בריאות עשויה '
...
NOTES = {
    ...
    15: 'לעבור על שש ההמלצות לפי מספרן, ולקשור כל אחת לתמה שממנה נגזרה. להבחין בין רמת המרפאה לרמת המדיניות.',
...
TABLE_ROWS = [
    ...
    ['1', 'ופאא דכוור', '54', 'רמלה', '12 שנות לימוד; סייעת בגן', 'נבדקה פעמיים, באיחור של כשנתיים'],
    ['2', 'ברכנש אלמו', '58', 'חולון', '4 שנות לימוד; עובדת ניקיון', 'נבדקה אחרי פנייה טלפונית באמהרית'],
    ['3', 'חנאן אבו סביח', '47', 'ערערה בנגב', 'תואר ראשון; מורה', 'נבדקת מתחת לגיל הזימון, בהפניית רופאה'],
    ['4', "איבתיסאם דאהר", '52', 'רמלה', '12 שנות לימוד; עובדת במספרה', 'בדיקה ראשונה בגיל 51 אחרי שנתיים'],
...
def png_ratio(name):
    b = open(os.path.join(W, name), 'rb').read(32)
    import struct as _s
    w, h = _s.unpack('>II', b[16:24])
    return h / w
...
def main():
    ...
    sub = txbody([
        para([run('מגישות:', sz=1300, b=True, color=MUTE)], algn='ctr', space_before=0),
        para([run('מראם זיד        ת.ז. 211794235', sz=1300)], algn='ctr', space_before=90),
        para([run('טגאיה סמאנך        ת.ז. 337622344', sz=1300)], algn='ctr', space_before=90),
    ...
    for n in range(2, 18):
        ...
        if n == 10:
            x = x.replace('</p:spTree>', pic_xml('rId3', 1.10, 2.82, 11.15, 11.15 * png_ratio('findings_path_wide.png'))
                          + '</p:spTree>')
