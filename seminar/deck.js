...
const HEB = /[\u0590-\u05FF]/;
const WJ = "\u2060"; // word joiner: keeps "\u05DB-80" or "\u05D1-SPSS" from breaking across lines
const glue = t => t.replace(/([\u0590-\u05FF])-(?=[0-9A-Za-z\u05D0-\u05EA])/g, `$1${WJ}-${WJ}`);
const rtl = t => typeof t === "string" && HEB.test(t)
  ? t.split("\n").map(l => (HEB.test(l) ? RLM + glue(l) + RLM : l)).join("\n")
  : t;
...
const para = (slide, text, x, y, w, h, size = 12) => T(slide, text, { x, y, w, h, fontSize: size, color: INK, align: "right", lineSpacingMultiple: 1.2 });
...
// ---------- 2. background ----------
{
  ...
  title(s, "רקע");
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 0.5, y: 1.1, w: 9, h: 3.95, fill: { color: WHITE }, line: { color: "B9CCE0", width: 1 }, rectRadius: 0.1 });
...
// ---------- 5. findings: population ----------
{
  ...
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 0.5, y: 1.1, w: 9, h: 3.95, fill: { color: WHITE }, line: { color: "F0B978", width: 1.25 }, rectRadius: 0.1 });
...
// ---------- 6. findings: correlation ----------
{
  ...
  T(s, "מתאם פירסון בין רווחה נפשית לבין תפיסת הלגיטימיות של הסגל, מובהק ברמה של p < 0.001 בקרב 2,344 שפוטים. מתאם ספירמן קרוב אליו ועומד על 0.34, ומכאן שההטיה בהתפלגות אינה מעוותת את התמונה.", { x: 6.25, y: 1.98, w: 2.95, h: 1.55, fontSize: 11.5, color: "D4E2F0", align: "right", lineSpacingMultiple: 1.15 });
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 0.5, y: 3.8, w: 9, h: 1.25, fill: { color: WHITE }, line: { color: "B9CCE0", width: 1 }, rectRadius: 0.1 });
